    ...
    const verifyCache = async () => {
      try {
        ...
        while (attempts < maxAttempts && !allReady) {
          ...
          const hasStaticCache = cacheNames.some(name => name.startsWith('iccat-v10'));
          const hasDataCache = cacheNames.includes('iccat-data-v10');
          ...
          if (hasDataCache) {
            const dataCache = await caches.open('iccat-data-v10');
          ...
          if (hasStaticCache) {
            const staticCache = await caches.open('iccat-v10');
