...
## Auto-Caching System (Offline-First)

### Real-Time Listener Auto-Caching
**How it works**: ALL CRUD operations (POST/PUT/DELETE) across all 10 collections automatically extract image URLs and pre-cache them in the background—completely non-blocking.

**Scenario**: 
1. Kiosk on screensaver (listener connections always open)
2. Admin adds building with image via dashboard
3. Backend broadcasts change to `/api/listen/buildings`
4. Frontend listener receives update → extracts image URLs → caches in background
5. Kiosk loses internet
6. User touches kiosk → new building + image load from cache ✅

**Covered Collections** (All auto-cache images from real-time broadcasts):
- Buildings (image field)
- Staff (photo/image fields)
- Events (image field)
- Floors (floorPlanImage field)
- Rooms (image/floorPlanImage fields)
- Walkpaths (any image references)
- Drivepaths (any image references)
- Indoor-nodes (icon/image fields)
- Room-paths (if image fields exist)
- Settings (if image fields exist)

**Image Fields Extracted**: image, photo, floorPlanImage, imageUrl, photoUrl, picture, icon, avatar

**Cache Strategy**:
1. **On app install**: Service Worker pre-caches all existing images
2. **On real-time updates**: Auto-cache new images from broadcasts
3. **On-demand viewing**: Cache images as user views them
4. **Service Worker fetch**: Cache-first for images, Network-first for API data
